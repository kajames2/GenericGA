#include "ranked_exponential_selector.h"
...
class RankedExponentialSelectorTest : public ::testing::Test {
public:
  RankedExponentialSelectorTest() {}
...
protected:
  virtual void SetUp() {
    ...
    sel = std::make_shared<genericga::RankedExponentialSelector<int, int>>();
  }
  genericga::Population<int, int> pop;
  std::shared_ptr<genericga::RankedExponentialSelector<int, int>> sel;
  std::shared_ptr<SampleFitnessCalculator> fit;
  std::shared_ptr<SamplePhenotypeConverter> phen;
};

TEST_F(RankedExponentialSelectorTest, CalculateWeightsTest) {
  auto vec = sel->CalculateWeights(pop);
  ASSERT_EQ(4, vec.size());
  ASSERT_DOUBLE_EQ(1-std::exp(-4), vec[1]);
  ASSERT_EQ(0.0, vec[3]);
  ASSERT_EQ(1-std::exp(-1.5), vec[2]);
}
