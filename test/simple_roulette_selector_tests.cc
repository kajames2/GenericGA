#include "simple_roulette_selector.h"
...
class SimpleRouletteSelectorTest : public ::testing::Test {
public:
  SimpleRouletteSelectorTest() {}
...
protected:
  virtual void SetUp() {
    ...
    sel = std::make_shared<genericga::SimpleRouletteSelector<int, int>>();
  }
  genericga::Population<int, int> pop;
  std::shared_ptr<genericga::SimpleRouletteSelector<int, int>> sel;
  std::shared_ptr<SampleFitnessCalculator> fit;
  std::shared_ptr<SamplePhenotypeConverter> phen;
};

TEST_F(SimpleRouletteSelectorTest, CalculateWeightsTest) {
  auto vec = sel->CalculateWeights(pop);
  ASSERT_EQ(4, vec.size());
  ASSERT_DOUBLE_EQ(6.0, vec[1]);
  ASSERT_EQ(-19.0, vec[3]);
  ASSERT_EQ(2.0, vec[2]);
}
