...
class SinglePopulationGA : public AbstractSinglePopulationGA<Phen> {
 public:
  ...
  std::vector<PhenotypeStrategy<Phen>> SelectStrategies(Selector& sel, int n) override {
    return pop_.SelectPhenotypeStrategies(sel, n);
  }
  PhenotypeStrategy<Phen> GetBestStrategy() override { return SelectStrategy(best_sel); }
