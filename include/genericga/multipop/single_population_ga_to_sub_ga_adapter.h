...
class SinglePopulationGAToSubGAAdapter : public AbstractSubGA<Environment> {
 public:
  ...
  PhenotypeStrategy<Phen> GetBestStrategy() {
    return ga_->GetBestStrategy();
  }
...
void SinglePopulationGAToSubGAAdapter<Environment, Phen>::SubmitPlayStrat(
    Environment& env) {
  env.AcceptStrategy(ga_->SelectStrategy(*selector_).phenotype,
                     this->GetID());
}
...
SinglePopulationGAToSubGAAdapter<Environment, Phen>::GetFitnessFunction(
  ...
  return [envs, id](const Phen& phen) -> float {
    return GetAccumulatedFitness(envs, id, phen);
  };
}

template <class Environment, class Phen>
float SinglePopulationGAToSubGAAdapter<Environment, Phen>::
    GetAccumulatedFitness(const std::vector<Environment>& envs, int id,
                          const Phen& phen) {
  float fit = std::accumulate(
      envs.begin(), envs.end(), 0.0,
      [phen, id](float tot, const Environment& env) -> float {
        return tot + env.GetFitness(phen, id);
      });
  return fit/envs.size();
}
